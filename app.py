...
if st.button("Generate Evaluation"):
    if not notes.strip():
        ...
        if evaluation_length == "1 minute":
            ...
            closing = "Overall, this was a strong and confident speech with good potential. With more focus on refining your delivery and engaging the audience, your future speeches will continue to improve. I look forward to seeing your progress."
        ...
        st.markdown(f"""
### Opening
Thank you, **{speaker_name if speaker_name else "Speaker"}**, for your speech{f" titled *{speech_title}*" if speech_title else ""}. {opening}
""")

        st.markdown("### Commendations")
        if strengths:
            for s in strengths[:2]:  # limit for time control
        ...
        if improvements:
            for i in improvements[:2]:  # limit for time control
